...
type User = {
  ...
  telegram_username?: string;
  telegram_id?: string;
  telegram_code?: string; // ✅ FIX ADDED
  active_promo?: {
    promo_code: string;
    discount: number; // ✅ FIX
    rules: string;
    ...
    <div className="flex items-center gap-2.5 mb-5">
      <div
        ...
        style={{
          ...
          border: `1px solid ${color}40`,
          clipPath:
            "polygon(0 0, 88% 0, 100% 30%, 100% 100%, 12% 100%, 0 70%)",
        }}
      >
        <Icon size={12} style={{ color }} />
      </div>
...
export default function ProfilePage() {
  ...
  const loadUser = async () => {
    try {
      ...
      // 🔥 FIX: discount logic
      setDiscount(Number(data?.active_promo?.discount || 0));
    } catch (err) {
      console.error(err);
    }
  };
  ...
  const applyPromo = async () => {
    try {
      ...
      if (data?.success) {
        ...
        setDiscount(Number(u?.active_promo?.discount || 0));
  ...
  return (
    <div className="min-h-screen text-white pb-24 pt-20" style={{ background: "#080809" }}>
      ...
      {/* EVERYTHING BELOW IS YOUR ORIGINAL UI — NOT TOUCHED */}

      <AnimatePresence>
        {promoNotice && (
          <motion.div className="fixed top-6 right-6 z-50 px-5 py-3"
            style={{ background: "#0D0D0F", border: "1px solid #FF3D0050" }}>
            <Zap size={13} />
            {promoNotice}
          </motion.div>
        )}
